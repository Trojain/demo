...
- 新增 `video` 视频解析模块，保持当前项目纯前端直连接口，不引入后端。
- 新增 `src/config/video.js`，集中维护 TikHub 接口域名、Bearer Token 配置位、抖音 `region=CN` 和合法域名说明。
- 新增 `src/services/video-api.js`，封装抖音高清播放链接接口和快手作品链接接口，并统一输出平台、标题、作者、封面、视频地址、时长和作品 ID。
- 新增 `pages/video/index` 页面，支持粘贴抖音或快手分享内容、自动识别平台、解析视频、预览播放、清空输入和保存到相册。
- 首页新增 `视频解析` 入口，并在 `pages.json` 注册视频解析页面路由。
- 更新“我的”页隐藏配置说明中的域名提示，补充 `api.tikhub.dev`。

已确认决策：

- 这是个人 demo 功能，TikHub Token 可以按用户确认直接写入前端配置文件。
- 当前不新增后端代理，不新增依赖，不引入额外 UI 框架。
- 抖音优先使用 TikHub 的高清播放链接接口，并传入 `region=CN`，尽量获取大陆访问更快的 CDN 链接。
- 快手先使用链接获取作品数据接口，真实返回字段通过服务层做兼容归一化。

待办和后期优化：

- 将真实 TikHub Token 填入 `src/config/video.js` 的 `VIDEO_API_CONFIG.token`。
- 使用真实抖音和快手分享链接验证 TikHub 返回结构，必要时继续补充字段兼容路径。
- 发布小程序前，需要在平台后台配置 `api.tikhub.dev`，以及 TikHub 返回的视频和封面资源真实域名。

已知坑位：

- TikHub 文档示例中 `data` 为占位 `null`，页面字段归一化已做多路径兼容，但仍需要真实接口返回确认。
- 小程序 `downloadFile` 保存视频时，视频资源域名必须进入 `downloadFile` 合法域名，否则保存会失败。
- 当前是纯前端 demo，Token 会打进小程序包，不适合放入有付费余额或需要保密的正式生产凭证。
...
- `video` 模块新增 TikTok 平台支持。
- 在 `VIDEO_PLATFORM_OPTIONS` 中补充 `TikTok` 展示项。
- 在视频解析服务中新增 TikTok 接口 `/api/v1/tiktok/app/v3/fetch_one_video_by_share_url`，请求参数按官方文档使用 `share_url`。
- 平台识别新增 `tiktok.com` 域名判断，覆盖 `www.tiktok.com`、`vm.tiktok.com`、`vt.tiktok.com` 等 TikTok 链接。
- 视频字段归一化补充 TikTok 常见结构，兼容 `itemInfo.itemStruct`、`video.playAddr`、`video.downloadAddr`、作者、封面、时长和作品 ID。
- 视频解析页文案更新为支持抖音、快手、TikTok 三个平台。

已确认决策：

- 当前继续保持纯前端直连 TikHub，不新增后端代理。
- TikTok 使用 App V3 分享链接接口，和抖音、快手共用同一套页面与保存到相册流程。

待办和后期优化：

- 使用真实 TikTok 分享链接验证返回结构，如 TikHub 实际字段和当前兼容路径不一致，再补充字段映射。
- 发布小程序前，需要把 TikTok 返回的视频和封面资源真实域名补充到 `downloadFile` 和图片合法域名配置中。
...
- 按 API 市场截图中的价格、免费额度信息调整 `video` 模块接口选择。
- 抖音从 `/api/v1/douyin/web/fetch_video_high_quality_play_url` 调整为 `/api/v1/douyin/app/v3/fetch_one_video_by_share_url`，避免默认使用更贵的高清播放链接接口。
- 快手从 `/api/v1/kuaishou/web/fetch_one_video_by_url` 调整为 `/api/v1/kuaishou/app/fetch_one_video_by_url`，请求参数改为 `share_text`。
- TikTok 继续使用 `/api/v1/tiktok/app/v3/fetch_one_video_by_share_url`。
- 删除不再使用的抖音 `region` 配置，避免误以为当前还在请求高清接口。

已确认决策：

- 每次转换无水印视频只允许请求一个接口。
- 当前不请求详情补充、统计、评论、用户主页、搜索、批量解析、高清兜底等非必要接口。
- 接口选择优先级为价格最低、满足单条无水印视频转换、截图中标记有免费额度。

已知坑位：

- 当前展示的标题、作者、封面、时长等信息只来自同一次解析接口返回，不会为了补齐展示字段额外请求接口。
- 如果单次接口未返回可下载视频地址，页面直接提示失败，不再自动调用第二个兜底接口。
...
- 优化 `video` 模块分享文案链接提取逻辑，支持从抖音、快手、TikTok 完整分享文案中自动提取平台链接。
- 链接提取优先选择抖音、快手、TikTok 域名，避免文案中混入其他链接时误取。
- 链接清洗会去掉末尾常见中文、英文标点，适配抖音文案中链接后跟口令和时间标记的场景。
- 视频解析页输入区新增“粘贴”按钮，调用小程序剪贴板能力读取分享文案。

已确认决策：

- 粘贴按钮只负责填充输入框，不自动发起解析，避免用户误触后直接消耗 TikHub 调用次数。
- 抖音、快手、TikTok 仍保持每次转换只请求一个接口。
...
- 在 `video` 页面输入区下方新增一条合并后的免责声明。
- 免责声明说明版权归平台及作者所有、本程序不存储内容、内容使用及侵权责任由使用人承担、服务按现状提供且不承担第三方内容责任。

已确认决策：

- 免责声明使用轻提示样式展示，不影响粘贴和解析主流程。
...
- 将底部 TabBar 从 `首页 / 我的` 调整为 `去水印 / 热门精选`。
- 将 `pages/video/index/index` 的去水印功能迁移到 `pages/index/index`，首页现在只承载去水印主流程。
- 新增 `pages/featured/index/index` 作为 `热门精选` Tab 页面，承载原首页的模块入口。
- 从 `pages.json` 中移除 `pages/video/index/index` 路由，并删除旧页面文件。
- `热门精选` 保留精选素材、热门影视、今日追剧入口，并保留原有注释中的高清壁纸入口代码。

已确认决策：

- `去水印` 作为第一个 Tab，打开小程序优先进入核心功能。
- 其他内容模块入口统一放入 `热门精选`，减少首页信息干扰。
...
- 新增 `src/utils/page-share.js`，统一生成微信好友分享和朋友圈分享参数。
- 为 `pages.json` 中 18 个路由页面全部补充 `onShareAppMessage` 和 `onShareTimeline`。
- 分享路径默认读取当前页面路由和 query，详情页、搜索页、分类页分享后可回到当前页面状态。
- 各页面分享标题按业务页面设置，如 `免费去水印`、`热门精选`、`精选素材`、`影视详情`、`剧集详情` 等。

已确认决策：

- 分享生命周期按 uni-app Options API 官方写法显式放在每个页面组件内。
- 公共工具只负责统一拼接当前页面路径和 query，页面仍保留自己的业务标题。

已知坑位：

- 朋友圈分享使用 `query` 字段，好友分享使用 `path` 字段，两者由公共工具分别生成。
- 分享到朋友圈能力主要面向微信小程序，其它端不作为本次验证重点。
