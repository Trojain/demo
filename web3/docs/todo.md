...
- ✅ 已完成风控记录直接归档 `strategyId` 和 `strategyVersionId`。
- ✅ 已完成恢复任务直接归档 `strategyId` 和 `signalId`。
- ✅ 已完成触发忽略、触发删除时释放待确认执行任务锁。
- ✅ 已完成快捷交易 `confirmToken` 幂等收口，覆盖模拟和真实交易确认。
- ✅ 已完成策略实例迁移自愈增强，支持旧库半迁移补齐。
